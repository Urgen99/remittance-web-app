...
import ContactUs from "../get-help/ContactUs";

const ReportBug = () => {
  const { activeStep, handleNext, handlePrev } = useStepper("report-bug");
  const handleNavigate = () => handlePrev("report-bug");
  return (
    <>
      {activeStep === "report-bug" && (
          ...
          <div className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-6 max-w-[24.75rem] w-full">
              ...
              <p className="font-roboto font-normal text-base leading-6 tracking-[-1%] text-[#1b1b1b]">
                ...
                <Button
                  variant="link"
                  onClick={() => handleNext("contact-us")}
                  className="underline text-[#3333C1] p-0 cursor-pointer"
                >
                  contact page
                </Button>{" "}
                for further help
      ...
      {activeStep === "contact-us" && <ContactUs handlePrev={handleNavigate} />}
    </>
  );
};

...
const Footer = () => {
  ...
  return (
    !disableFooterPaths.some((link) => matchPath(link, pathname)) && (
      <footer className="flex justify-between items-end relative">
        <SlantedBlock />
...
const SlantedBlock = ({ reversed = false }: { reversed?: boolean }) => (
  <div
    className={`absolute bottom-0 transition-all ease-in-out duration-300 w-0 h-0 sm:border-r-[10rem] sm:border-b-[7.5rem] md:border-r-[15rem] md:border-b-[9.5rem] lg:border-r-[20rem] xl:border-r-[27.5rem] xl:border-b-[10.3rem] border-b-[#2E2EA0] border-white ${
      reversed ? "scale-x-[-1] right-0" : "left-0"
    }`}
  />
);
